...
exiftool $LineGotif |awk '/Keywords/'|sed 's/Keywords                        \: //g'|sed 's/THIS_IS_A_LINEBREAK./\
/g' |sed 's/Keywords                        \: //g' |sed 's/THIS_IS_A_LINEBREAK./\
/g'|awk 'NR == 2'>> tmp/exiftool
done
headerlocation=$(cat tmp/FoundTiFTagged|awk 'NR == 1')
exiftool $headerlocation |awk '/Keywords/'|sed 's/Keywords                        \: //g'|sed 's/THIS_IS_A_LINEBREAK./\
/g' |sed 's/Keywords                        \: //g' |sed 's/THIS_IS_A_LINEBREAK./\
...
#geoserverworkspace=$(echo "$RowWithHeader"| awk -F'|' 'NR==1 {for (i=1; i<=NF; i++) {f[$i] = i}}; { print $(f["geoserverworkspace"]) }' OFS='|' |awk 'NR == 2')
...
#
#echo "$purple CHAR3 planchesNames $planchesNames"
#
#echo NodeID=\"$NodeID\" >> tmp/tmp_bash
#elif [[ $PlancheName_Simple == Feuille-* ]]
#then
#
#PlancheFeuille=$( echo "$PlancheName_Simple"| sed 's/Feuille-//g' |awk -F'_' '{print $1}')
...
#NordOuest=$(echo "$Nord $Ouest")
#SudOuest=$(echo "$Sud $Ouest")
#SudEst=$(echo "$Sud $Est")
#NordEst=$(echo "$Nord $Est")
#
#else
...
#
#echo "$lightblue DEBUG BC BUG A"
#
#NordBasic=$(echo 126224-$Hauteur*$AbscissaMultiple |bc -l)
#echo "$lightblue DEBUG BC BUG B"
#SudBasic=$(echo 125824-$Hauteur*$AbscissaMultiple|bc -l )
#echo "$lightblue DEBUG BC BUG C"
#EstBasic=$(echo 600000+\($OrdinateMultiple*$Largeur\) |bc -l )
#echo "$lightblue DEBUG BC BUG D"
#OuestBasic=$(echo 599400+\($OrdinateMultiple*$Largeur\) |bc -l )
#echo "$lightblue DEBUG BC BUG E"
#echo "$green$LayerBound"
#echo "${white}Layer extent 27561  $Nord|$Sud|$Est|$Ouest"
#echo "Basic  extent 27561 $NordBasic|$SudBasic|$EstBasic|$OuestBasic"
#echo "$lightblue DEBUG BC BUG Result
#NordBasic   $NordBasic
#SudBasic    $SudBasic
#EstBasic    $EstBasic
#OuestBasic  $OuestBasic
#AbscissaMultiple $AbscissaMultiple
#OrdinateMultiple $OrdinateMultiple
#Abscissa $Abscissa
#Ordinate $Ordinate
#"
#
...
#
#
#echo "${white}---> \$base_name $base_name"
#echo planche simple $PlancheName_Simple
#
#
#
#echo "$Filename|$NomMachine|$NordOuest|$SudOuest|$SudEst|$NordEst|$SudOuest4326|$SudEst4326|$NordEst4326 |$LayerBound   $Abscissa|$Ordinate|"
#
#
#
#
#
#echo "${bg_white}${black}---> Good Bye ! from SubRoutine_forNode.sh${reset}"
#
#
#
##
